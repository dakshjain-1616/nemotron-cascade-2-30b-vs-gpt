...
Usage:
  python battle.py --count 50
  python battle.py --count 5 --mock      # force mock mode
  python battle.py --count 10 --no-html  # JSON only
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Arduino AI Battle — benchmark Nemotron vs GPT-Nano on real forum bugs",
...
def print_summary(results: list[EvalResult]) -> None:
    ...
    table.add_column("GPT-Nano", style="cyan")
    ...
    if len(categories) > 1:
        ...
        cat_table.add_column("GPT-Nano Wins", style="cyan")
